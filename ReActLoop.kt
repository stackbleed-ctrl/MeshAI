...
class ReActLoop @Inject constructor(
    ...
    suspend fun execute(
        ...
        executionMutex.withLock {
            ...
            while (step < MAX_STEPS && finalAnswer == null) {
                ...
                val response = llmEngine.complete(systemPrompt, history)
                ...
                if (toolCall != null) {
                    ...
                    }.getOrElse { ToolResult.failure("Tool error: ${it.message}") }
                    ...
                    val observation = when (validation) {
                        ...
                        is ValidationResult.Invalid -> "Observation: ${validation.sanitized}"
                    }
                    history.add(LlmMessage(LlmRole.USER, observation))

                    val toolCost = budget.chargeTool(toolInput, toolResult.summary)
                    agentMemory.store("task_${task.taskId}_step_$step",
                        "Tool=$toolName|Result=${toolResult.summary.take(200)}")
                    ...
                    history.add(LlmMessage(LlmRole.USER,
                        "Continue. Prefix final answer with '$FINAL_ANSWER_SIGNAL'"))
            ...
            val trace = ExecutionTrace(task.taskId, task.title, stepTraces, result, outcome,
                budget.snapshot(), System.currentTimeMillis() - startMs)
    ...
    private fun pruneHistory(history: MutableList<LlmMessage>) {
        ...
        val summary = "Summary of previous steps: " + toSummarise.mapIndexed { i, m ->
            "[${if (m.role == LlmRole.ASSISTANT) "Agent" else "Obs"} ${i+1}] ${m.content.take(120).replace('\n', ' ')}"
        }.joinToString("; ")
        toSummarise.clear()
